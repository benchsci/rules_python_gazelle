...
func findRuleWithOverride(c *config.Config, mod module, from label.Label, explainDependency string, languageName string) (*treeset.Set, bool) {
	deps := treeset.NewWith(godsutils.StringComparator)
	imp := resolve.ImportSpec{Lang: languageName, Imp: mod.Name}
	imports := make([]resolve.ImportSpec, 0, len(mod.SubName)+2)

	imports = append(imports, imp)

	for _, name := range mod.SubName {
		imp := resolve.ImportSpec{Lang: languageName, Imp: fmt.Sprintf("%s.%.s", mod.Name, name)}
		imports = append(imports, imp)
	...
	return deps, !deps.Empty()
}

func findRulesByImportWithConfig(c *config.Config, ix *resolve.RuleIndex, mod module, lang string) []resolve.FindResult {

	imp := resolve.ImportSpec{Lang: languageName, Imp: mod.Name}

	allMatches := make([]resolve.FindResult, 0, len(mod.SubName)+1)

	for _, name := range mod.SubName {
		imp := resolve.ImportSpec{Lang: languageName, Imp: fmt.Sprintf("%s.%s", mod.Name, name)}
		allMatches = append(allMatches, ix.FindRulesByImportWithConfig(c, imp, lang)...)

	}
	if len(allMatches) > 0 {
		return allMatches
	}
	return ix.FindRulesByImportWithConfig(c, imp, lang)
}
		...
		for it.Next() {
			...
			if overrides, ok := findRuleWithOverride(c, mod, from, explainDependency, languageName); ok {
				...
				if dep, moduleName, ok := cfg.FindThirdPartyDependency(mod.Name); ok {
					...
					matches := findRulesByImportWithConfig(c, ix, mod, languageName)
					if len(matches) == 0 {
